...
if(isset($_POST['submit'])){
    ...
    $select = mysqli_query($conn, "SELECT * FROM `utilisateurs` WHERE email = '$email' AND password = '$pass'") or die('query failed');
 
    if(mysqli_num_rows($select) > 0){
       $message[] = 'Cet utilisateur existe déja !';
    }else{
       mysqli_query($conn, "INSERT INTO `utilisateurs`(name, email, password) VALUES('$name', '$email', '$pass')") or die('query failed');
...
?>
   ...
   <div class="form-container">
      <form action="" method="post">
         ...
         <input type="password" name="cpassword" required placeholder="confirmer le mot de passe" class="box">
         <input type="submit" name="submit" class="btn" value="Inscrivez vous">
